...
// Audi four-rings logo (inline SVG)
function AudiRings() {
  const r = 7.2, gap = 5.6, cy = 20;
  const centers = [20 - gap * 1.5, 20 - gap * 0.5, 20 + gap * 0.5, 20 + gap * 1.5];
  return (
    <svg width="40" height="40" viewBox="0 0 40 40" fill="none">
      <rect width="40" height="40" rx="4" fill="#BB0A14" />
      {centers.map((cx, i) => (
        <circle key={i} cx={cx} cy={cy} r={r} stroke="white" strokeWidth="2.2" fill="none" />
      ))}
    </svg>
  );
}
