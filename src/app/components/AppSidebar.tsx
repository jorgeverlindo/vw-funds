...
function ClientLogoImg({ src, alt, scale = 1, noPadding = false }: { src: string; alt: string; scale?: number; noPadding?: boolean }) {
  // [FV] white square with 5px horizontal padding so the mark is centered and never touches the edge.
  // `scale` lets specific marks (e.g. VW OEM, whose PNG has internal whitespace) be enlarged so the
  // visible mark itself sits inside the 5px padding rather than the PNG bounding box.
  return (
    <div
      style={{
        width: 40,
