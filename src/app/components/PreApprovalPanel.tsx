import { X, Eye, Star, Users, Download } from 'lucide-react';
...
export function PreApprovalPanel({ preApproval, onClose }: PreApprovalPanelProps) {
      ...
      <div className="flex items-center justify-between px-8 py-5 border-b border-[#E0E0E0] shrink-0">
        <div className="flex flex-col">
          <h2 className="text-[#1f1d25] text-[20px] font-medium tracking-[0.15px] leading-tight">
            {t('Pre-approval')} - {preApproval.id} ({preApproval.mediaType.split('/')[0].split(' ')[0]})
          </h2>
        </div>
        ...
        <div className="flex items-center gap-3">
           ...
           <div className="flex items-center gap-2">
             ...
             {/* Status Chip */}
             <StatusChip status={preApproval.status} />
           </div>
        ...
        <div className="px-8 py-6 space-y-8">
          ...
          <section>
            ...
            <div className="space-y-0">
              <KeyValueRow label={t("Last updated")} value={preApproval.lastUpdated.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })} />
              <KeyValueRow label={t("Submitted at")} value={preApproval.submittedAt.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })} />
              <KeyValueRow label={t("Submitted by")} value={preApproval.submittedBy.name} />
              <KeyValueRow label={t("Dealership")} value={`${preApproval.dealershipCode} - ${preApproval.dealershipName} (${preApproval.dealershipCity})`} />
              <KeyValueRow label={t("Initiative Type")} value={t(preApproval.initiativeType)} />
              <KeyValueRow label={t("How many claims will you be making")} value={preApproval.claimsCount.toString()} />
              <KeyValueRow 
          ...
          {/* Media Type Section */}
          <section>
            <h3 className="text-[#1f1d25] text-xs font-bold uppercase mb-3 tracking-[0.8px] text-[#686576]">{t('MEDIA TYPE')}</h3>
          ...
          <section>
            ...
            {preApproval.documents.length > 0 ? (
              <div className="border border-[#E0E0E0] rounded-lg p-0 flex overflow-hidden bg-white max-w-md">
                 ...
                 <div className="flex-1 p-3 flex items-center justify-between min-w-0">
                   <div className="flex flex-col min-w-0 pr-3">
                     <span className="text-[#1f1d25] text-[13px] font-medium truncate mb-0.5">{preApproval.documents[0].name}</span>
                     <span className="text-[#686576] text-[11px] uppercase tracking-wide">{preApproval.documents[0].type} | {preApproval.documents[0].size}</span>
                   </div>
           ...
           {/* Revisions */}
           {preApproval.status === 'Revision Requested' && (
