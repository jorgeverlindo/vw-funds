...
// Ground plane Y fraction for DEALER custom backgrounds, derived per template.
// These backgrounds are generated by nano-banana with a KNOWN ground geometry
// (the prompt fixes where asphalt starts per format), so no depth detection is
// needed — the tire line is deterministic:
//   wide banners: ground band spans 30%→100% of height → tires at ~80%
//   portrait:     large foreground → tires at ~72%
//   square/std:   ground starts at 60% → tires at ~66%
// Catalog backgrounds return the bg's own groundFraction (usually undefined →
// original CSS positioning), keeping that flow untouched.
function dealerGroundFraction(
  bg: CustomBackground | null,
  template: Template,
): number | undefined {
  if (!bg?.id?.startsWith("dealer-bg-")) return bg?.groundFraction;
  const ar = template.width / template.height;
  if (ar > 2.0) return 0.80;
  if (ar < 0.7) return 0.72;
  return 0.66;
}
              ...
              {visibleTemplates.flatMap(template =>
                ...
                >
                  <JellyBeanCard
                    ...
                    brandKit={brandKit}
                    groundFraction={dealerGroundFraction(selectedBg as CustomBackground | null, template)}
                ...
                return (
                  ...
                  >
                    <JellyBeanCard
                      ...
                      brandKit={brandKit}
                      groundFraction={dealerGroundFraction(bg as CustomBackground | null, template)}
