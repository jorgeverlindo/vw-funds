...
import {
  ...
  BarChartSvg, Legend, EndMarker,
} from '../components/Primitives';
import { fmtUSD, fmtPct, groupSum, dealerName, barWidth } from '../../reports/utils';
  ...
  return (
    <Document>
      ...
      <ReportPage reportTitle={TITLE} period={PERIOD} pageNum={2}>
        ...
        <Lead>
          Your VW network processed <Bold>{n} claims</Bold> across the reporting window, with an average total cycle of{' '}
          <Bold>{avgTotal.toFixed(1)} days</Bold> from submission to payment. Phase 1 (submission to pre-approval) is the longest at{' '}
          <Bold>{avgP1.toFixed(1)} days</Bold>.
        </Lead>
        ...
        <Body>
          ...
          The biggest lever for reducing total cycle time is <Bold>Phase 1</Bold>, which accounts for <Bold>{fmtPct((avgP1 / avgTotal) * 100)}</Bold> of total cycle time.
        </Body>
      </ReportPage>
