...
import {
  ...
  KpiGrid, BreakdownTable, StatStrip, Callout, TwoCol,
  DonutSvg, Legend, Divider, EndMarker,
} from '../components/Primitives';
import { fmtUSD, fmtPct, groupSum, sortedEntries, dealerName } from '../../reports/utils';
  ...
  return (
    <Document>
      ...
      <ReportPage reportTitle={TITLE} period={PERIOD} pageNum={2}>
        ...
        <Lead>
          Of the <Bold>{fmtUSD(totalAccrued)}</Bold> accrued across the period,{' '}
          <Bold>{fmtUSD(available)} ({fmtPct(availablePct)}) remains available</Bold> and{' '}
          <Bold>{fmtUSD(totalPaid)} ({fmtPct(reimbursedPct)}) has been reimbursed</Bold>.
          A further <Bold>{fmtUSD(totalPending)}</Bold> sits in pending claims awaiting resolution.
        </Lead>
        ...
        <Body>
          ...
          Your network currently tracks at <Bold>{fmtPct(reimbursedPct)}</Bold>, indicating solid claim flow with real activation room still available.
        </Body>
      </ReportPage>
          ...
          left={
            <>
              ...
              <Body style={{ marginBottom: 18 }}>
                Of the total accrued balance, <Bold>{fmtPct(availablePct)} remains available</Bold> and{' '}
                <Bold>{fmtPct(reimbursedPct)} has been reimbursed</Bold>. A near-even split signals healthy claim flow without funds languishing.
              </Body>
              <Legend items={[
