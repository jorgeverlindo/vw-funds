...
import {
  ...
  DonutSvg, Legend, Divider, ClosingGrid, EndMarker,
} from '../components/Primitives';
import { fmtPct, groupSum, sortedEntries } from '../../reports/utils';
  ...
  return (
    <Document>
      ...
      <ReportPage reportTitle={TITLE} period={PERIOD} pageNum={2}>
        ...
        <Lead>
          Your VW network submitted <Bold>{totalPA.toLocaleString()} pre-approvals</Bold> across the reporting window.{' '}
          <Bold>{fmtPct(approvalRate)} were approved</Bold> on first review, while{' '}
          <Bold>{fmtPct(declineRate)}</Bold> were declined. A small share required revision before final decision.
        </Lead>
        ...
        <Body>
          An approval rate above 80% indicates a well-trained dealer network that understands eligibility criteria. Your <Bold>{fmtPct(approvalRate)}</Bold> rate is a strong signal — the 12% decline rate is worth monitoring to identify whether it clusters around specific dealers, fund types, or time periods.
        </Body>
      </ReportPage>
