...
import { SectionTag, SectionTitle, Em, Lead, Body, SubHead, KpiGrid, BreakdownTable, ForecastTable, Callout, TwoCol, DonutSvg, Legend, Divider, ClosingGrid, EndMarker } from '../components/Primitives';
import { fmtUSD, fmtPct, fmtDate, groupSum, barWidth } from '../utils';
  ...
  return (
    <div>
      ...
      <ReportPage reportTitle={TITLE} period={PERIOD} pageNum={2}>
        ...
        <Lead>
          Your fund pool is split across <strong style={{ color: C.text }}>{fundEntries.length} fund programs</strong>, with a total contracted value of{' '}
          <strong style={{ color: C.text }}>{fmtUSD(totalValue)}</strong>. Media costs account for{' '}
          <strong style={{ color: C.text }}>{fmtPct(mediaPct)}</strong> of contracted budget, with hard costs covering the remaining <strong style={{ color: C.text }}>{fmtPct(hardPct)}</strong>.
        </Lead>
