...
import { SectionTag, SectionTitle, Em, Lead, Body, SubHead, KpiGrid, BreakdownTable, StatStrip, Callout, Divider, BarChartSvg, EndMarker, Legend } from '../components/Primitives';
import { fmtUSD, fmtPct, fmtMonthLabel, groupSum, sortedEntries } from '../utils';
  ...
  return (
    <div>
      ...
      <ReportPage reportTitle={TITLE} period={PERIOD} pageNum={2}>
        ...
        <Lead>
          Across the full period, your VW network accrued <strong style={{ color: C.text }}>{fmtUSD(totalAccrued)}</strong> in co-op funds and submitted{' '}
          <strong style={{ color: C.text }}>{fmtUSD(totalSubmitted)}</strong> in claims —{' '}
          a submission rate of <strong style={{ color: C.text }}>{fmtPct((totalSubmitted / totalAccrued) * 100)}</strong>. Of submitted claims,{' '}
          <strong style={{ color: C.text }}>{fmtPct(approvalRate)}</strong> were approved.
        </Lead>
        ...
        <Body style={{ maxWidth: '620px' }}>
          The gap between accrued and submitted funds represents untouched budget that dealers have earned but not yet activated. At <strong style={{ color: C.text }}>{fmtPct(100 - (totalSubmitted / totalAccrued) * 100)}</strong>, this is the activation opportunity your OEM team can address through co-op outreach.
        </Body>
      </ReportPage>
