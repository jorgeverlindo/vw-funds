...
import { SectionTag, SectionTitle, Em, Lead, Body, SubHead, KpiGrid, ForecastTable, StatStrip, Callout, Divider, BarChartSvg, Legend, ClosingGrid, EndMarker } from '../components/Primitives';
import { fmtUSD, fmtPct, fmtDate, daysBetween, barWidth } from '../utils';
  ...
  return (
    <div>
      ...
      <ReportPage reportTitle={TITLE} period={PERIOD} pageNum={2}>
        ...
        <Lead>
          Of the <strong style={{ color: C.text }}>{fmtUSD(totalVolume)}</strong> in total claim payment volume,{' '}
          <strong style={{ color: C.text }}>{fmtUSD(totalPaid)}</strong> has been disbursed across <strong style={{ color: C.text }}>{paidTxns.length} paid transactions</strong>.
          The average payment lag from submission to disbursement is <strong style={{ color: C.text }}>{avgLag.toFixed(1)} days</strong>,
        ...
        <Body style={{ maxWidth: '620px' }}>
          ...
          <strong style={{ color: C.text }}>{avgLag.toFixed(1)} days</strong> — watch for outlier transactions above 25 days that may signal processing bottlenecks.
        </Body>
      </ReportPage>
